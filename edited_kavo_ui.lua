...
function Elements:NewSearchBox(tname, placeholderText)
    tname = tname or "Search"
    placeholderText = placeholderText or "Search elements..."
    
    -- Create the search textbox
    local searchElement = Instance.new("TextBox")
    searchElement.Parent = page  -- Ensure it gets added correctly
    searchElement.BackgroundColor3 = themeList.ElementColor
    searchElement.Size = UDim2.new(0, 352, 0, 33)
    searchElement.Font = Enum.Font.FredokaOne
    searchElement.PlaceholderText = placeholderText
    searchElement.Text = ""
    searchElement.TextColor3 = themeList.TextColor
    searchElement.TextSize = 14.000
    searchElement.ClearTextOnFocus = false

    -- Create a UI Corner for rounded edges
    local UICorner = Instance.new("UICorner")
    UICorner.CornerRadius = UDim.new(0, 4)
    UICorner.Parent = searchElement

    -- Debugging: Check if it's being created
    print("Search box created!")

    -- Function to filter elements based on search
    searchElement:GetPropertyChangedSignal("Text"):Connect(function()
        local searchText = searchElement.Text:lower()
        print("Searching for:", searchText)  -- Debugging

        for _, child in pairs(page:GetChildren()) do
            if child:IsA("Frame") or child:IsA("TextButton") then
                local label = child:FindFirstChild("togName") or child:FindFirstChild("btnInfo") or child:FindFirstChild("TextLabel")
                if label then
